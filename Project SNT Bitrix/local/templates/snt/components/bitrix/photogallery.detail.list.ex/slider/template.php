...
<div class="photo-items-list photo-photo-list gallery__images"
    ...
    foreach ($arResult["ELEMENTS_LIST"] as $key => $arItem) {
        ...
        ?>
        <div id="photo_cont_<?= $arItem["ID"] ?>" class="photo-item-cont <? if ($arParams["PERMISSION"] >= "X") {
            echo ' photo-item-cont-moder';
        } ?>" title="<?= $arItem["TITLE"] ?>">
            <a class="photo-item-inner"
               ...
               href="<?= $arItem["URL"] ?>" id="photo_<?= $arItem["ID"] ?>">
                <div border="0"
                     style="<?= $item_w ?> <?= $item_h ?> <?= $item_top ?> background: url('<?= $src ?>'); background-size: cover"
                     ; alt="<?= $alt ?>"></div>
                <? if ($bNotActive):?>
...
<? if ($arResult["MORE_PHOTO_NAV"] == "Y"): ?>
    <div id="photo-more-photo-link-cont-<?= $ucid ?>" class="photo-show-more">
        ...
        <a id="photo-more-photo-link-<?= $ucid ?>" href="javascript:void(0);"
           title="<?= GetMessage("P_SLIDER_MORE_PHOTOS_TITLE") ?>"><?= GetMessage("P_SLIDER_MORE_PHOTOS") ?></a>
    </div>
<? endif; ?>
