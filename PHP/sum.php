...
/* c = "<br><table><thead><tr><td>Дата операции</td><td>Тип операции</td><td>Цель операции</td><td>Сумма операции</td><td>Валюта</td></tr></thead><tbody><?foreach ($arrEditedCons as $data):?><tr><?foreach ($data as $str):?><td><?=substr($str,(strpos($str, ':')+1),-2)?></td><?endforeach;?></tr><?endforeach;?></tbody></table><p class='table_footer'>Общая сумма расходов в $ составляет: <?=$USA?>.</p><p class='table_footer>'Общая сумма расходов в € составляет: <?=$EURO?>.</p><p class='table_footer'>Самые распространенные расходы: </p><ul><?foreach($max as $k=> $value):?><li><?=$k . " " . $value . ' раз'?>.</li><?endforeach;?></ul>            <br><br><table><thead><tr><td>Дата операции</td><td>Тип операции</td><td> Цель операции</td><td>Сумма операции</td><td>Валюта</td></tr></thead><tbody><?foreach ($arrEditedIncome as $data):?><tr><?foreach ($data as $str):?><td><?=substr($str,(strpos($str, ':')+1),-2)?></td><?endforeach;?></tr><?endforeach;?></tbody></table><p class='table_footer'>Общая сумма доходов в $ составляет: <?=$USAIn?>.</p><p class='table_footer'>Общая сумма доходов в € составляет: <?=$EUROIn?>.</p><br><p class='table_footer'>Разница Доходов/Расходов в $ составляет: <?=$USAIn - $USA?>.</p><p class='table_footer'>Разница Доходов/Расходов в € составляет: <?=$EUROIn - $EURO?>.</p>" */
?>
<br>
<table>
    <thead>
